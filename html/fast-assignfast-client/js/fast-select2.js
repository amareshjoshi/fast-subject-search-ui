...
function select2Ajax(selectId, facet) {
    ...
    $(selectId).select2({
        ...
        ajax: {
            ...
            processResults: function (data) {
                ...
                // take arrayFast and just add an "id": ??? filed to each element
                function simpleSelect(value, index) {
                    var foo = value;
                    foo.id = index;
                    arraySelect2.items[index] = foo;
                }
                //-------------------------------------------------------------------------
                return {
    ...
    function formatSubject(subject) {
        ...
        var $subject = $(
            ...
            `<span>Form Select = ${subject[facet][0]}</span><span>,&nbsp;</span>` +
            `<span>FAST Suggested = <span><b>${subject["auth"]}</b></span></span><br>` +
            `<span>FASTID = ${subject["idroot"]}</span><span>,&nbsp;</span>` +
            `<span>Facet =  ${getTypeFromTag(subject["tag"])}</span><hr>`
        );
        return $subject;
    ...
    function formatSubjectSelection(subject) {
        if (subject.auth) {
            ...
            jQuery("#exampleXtra").html(
                "Related Metadata:<br>" +
                `FAST ID: <span><b>${subject["idroot"]}</b></span><br>` +
                `Facet (type): <span><b>${getTypeFromTag(subject["tag"])}</b></span><br>` +
                `tag: <span><b>${subject["tag"]}</b></span><br>` +
                `auth: <span><b>${subject["auth"]}</b></span><br>` +
            ...
            var $subject = $(
                ...
                `<span>${subject["auth"]}</span><span>&nbsp;(Selected = ${subject[facet][0]})</span>`
            );
            return $subject;
        }
