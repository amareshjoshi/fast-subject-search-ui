...
function autoSubject(request, response, responseStyle) {
  ...
  var suggestReturn =
    suggestIndex + "%2Cidroot%2Cauth%2Ctag%2Ctype%2Craw%2Cbreaker%2Cindicator";
...
function connexStyle(res) {
  ...
  } else if (res["tag"] == 151) text = text.replace(/\$z/g, sep + "z ");
  else if (res["tag"] == 155) text = text.replace(/\$v/g, sep + "v ");
  else if (res["tag"] == 100) {
    text = text.replace(/\$d/g, sep + "d ");
    text = text.replace(/\$b/g, sep + "b ");
    text = text.replace(/\$c/g, sep + "c ");
    text = text.replace(/\$q/g, sep + "q ");
  } else if (res["tag"] == 110) text = text.replace(/\$b/g, sep + "b ");
  else if (res["tag"] == 130) {
